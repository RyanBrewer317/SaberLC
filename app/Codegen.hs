...
go stmts =
    ...
    let stmts_ops = NewRgnOp : GlobalFuncOp (-1) : CallOp : EndFunctionOp : reverse stmts_ops_rev in
...
goStmt (Func id tvars params body) =
    let (ct_stack_size, tvar_ops_rev, ct_locals) = foldr (\x (i, ops_rev, locals) ->
            ...
            ) (Size 1, [], Locals empty) (reverse $ ids tvars) in
    let (_, params_ops_rev, rt_locals) = foldr (\(x, t) (i, ops_rev, locals) ->
            ...
            ) (Size (length params + 1), [], Locals empty) (reverse params) in
    let body_ops = goExpr (inc $ Size $ length params) (inc ct_stack_size) (trace (show rt_locals) rt_locals) ct_locals body in
...
goExpr rt_stack_size ct_stack_size rt_locals ct_locals e = case e of
    ...
    MallocCPS () x ts (Rgn handle) scope ->
        let handle_ops = goVal rt_stack_size ct_stack_size rt_locals ct_locals (VarCPS handle undefined False) in
        ...
        scope_ops++MallocOp:TupleOp (length ts_ops):ts_ops++handle_ops
