...
goVal v = case v of
   ...
   f@(LambdaCPS _ tvars args body) -> do
      let ys = fvVal [] f
      let ts = ftvVal [] f
      t_env <- goType $ ProductCPS $ map (\(_,t,_)->(PreAlloc,t)) ys
      ...
      let v_code = LambdaCPS NotFalse (ts++tvars) args2 $
            foldr (\(i,(x,_t,_global)) e->TupleProjCPS x (VarCPS z_env t_env False) i e) body2 $ zip [0..] ys
      v_env <- TupleCPS NotFalse <$> mapM (\(x,t,global)->goType t >>= \t2->return (PreAlloc,VarCPS x t2 global)) ys
...
fv :: [Id] -> ExprCPS V V V -> [(Id, TypeCPS V V V, Bool)]
...
fvVal :: [Id] -> ValCPS V V V -> [(Id, TypeCPS V V V, Bool)]
