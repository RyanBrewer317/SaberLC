...
goExpr e k = case e of
    ...
    Lambda x t body -> do
        ...
        return $ AppCPS k [] [LambdaCPS [] [(x, goType t), (cont, cont_t)] body2]
    App _t e1 e2 -> do
        x1 <- fresh
        x2 <- fresh
        cont <- goExpr e2 $ LambdaCPS [] [(x2, goType $ getType e2)] $
                    AppCPS (VarCPS x1 $ goType $ getType e1) [] [VarCPS x2 $ goType $ getType e2, k]
        goExpr e1 $ LambdaCPS [] [(x1, goType $ getType e1)] cont
    TLambda x body -> do
        cont <- fresh
        let cont_t = ArrowCPS [] [goType $ getType body]
        body2 <- goExpr body $ VarCPS cont cont_t
        return $ AppCPS k [] [LambdaCPS [x] [(cont, cont_t)] body2]
    TApp _t e1 targ -> do
        x <- fresh
